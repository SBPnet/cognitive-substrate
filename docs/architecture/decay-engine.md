---
title: Decay Engine Architecture
...
The implemented source surface is `packages/decay-engine/`. `docs/architecture/inventory.md` currently treats this stage conservatively; most Series II engines have package entrypoints and focused or entrypoint evidence, not longitudinal behavioral proof.

## Purpose

This component models memory decay, suppression, and retention pressure. The architecture treats the name as a functional engineering label, not a biological or psychological equivalence claim.

## Runtime Surface

Review `packages/decay-engine/src/index.ts` and neighboring files for the current exported API. Runtime wiring varies by stage; do not assume a Kafka topic, worker, or OpenSearch index exists unless it is declared in `packages/kafka-bus/src/topics.ts`, `packages/memory-opensearch/src/schemas.ts`, or an app under `apps/`.
