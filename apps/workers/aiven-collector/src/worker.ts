...
export async function startWorker(): Promise<void> {
  // Per-worker mutable state; scoped here to keep the module stateless and
  // allow future multi-instance or test instantiation without shared state.
  ...
  const logsPoll = (): Promise<void> => collectLogs(config, client, producer, logOffsets, log);
  const metricsPoll = (): Promise<void> => collectMetrics(config, client, producer, log);
  const eventsPoll = (): Promise<void> =>
    collectProjectEvents(config, client, producer, seenProjectEvents, log);
